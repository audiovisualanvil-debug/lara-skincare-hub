...
const categoryConfig: Record<string, { label: string; icon?: typeof Droplets }> = {
  ativos: { label: "Ativos Skin Pro", icon: Droplets },
  corporal: { label: "Corporal", icon: Activity },
  peelings: { label: "Peelings", icon: FlaskConical },
  microagulhamento: { label: "Microagulhamento", icon: Syringe },
};
...
const SmartGRPage = () => {
  ...
  const { products, isLoading, categories: catCounts } = useBrandProducts("Smart GR");

  const dynamicCategories = useMemo(() => {
    const cats = [{ id: "all", label: "Todos", count: products.length, icon: undefined as typeof Droplets | undefined }];
    ...
    <>
      ...
      <main className="min-h-screen bg-background">
        ...
        <section className="relative h-[45vh] md:h-[55vh] overflow-hidden">
          ...
          <motion.div className="absolute inset-0 bg-gradient-to-r from-indigo-900/85 via-indigo-900/60 to-transparent" style={{ opacity }} />
          <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent" />
          <div className="relative h-full container mx-auto px-4 flex items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-2xl">
              <Badge className="mb-4 bg-indigo-500/20 text-indigo-300 border-indigo-500/30 backdrop-blur-sm">Estética Profissional</Badge>
              ...
              <p className="text-lg md:text-xl text-white/80 mb-6 leading-relaxed max-w-lg">
                Tecnologia Bio Green e Octahidro para resultados profissionais. Ativos de alta performance para protocolos clínicos avançados.
              </p>
              ...
              <div className="flex flex-wrap gap-4">
                <Button size="lg" className="bg-indigo-600 hover:bg-indigo-700 text-white">Ver Ativos</Button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <motion.div key={feature.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + index * 0.1 }} className="flex items-center gap-3">
                  <div className="p-2 bg-indigo-500/10 rounded-lg"><feature.icon className="w-5 h-5 text-indigo-600" /></div>
              ...
              {dynamicCategories.map((cat) => (
                <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 flex items-center gap-2 ${
                    activeCategory === cat.id ? "bg-indigo-600 text-white shadow-md" : "bg-secondary text-secondary-foreground hover:bg-indigo-500/10"
                  }`}
                >
        ...
        <AnimatedSection direction="fade" className="bg-indigo-700">
          <div className="container mx-auto px-4 py-16 text-center">
            <h2 className="text-2xl md:text-3xl font-semibold text-white mb-4">Quer conhecer nossos ativos inteligentes?</h2>
            <p className="text-white/80 mb-8 max-w-xl mx-auto">Entre em contato para saber mais sobre nossas soluções dermatológicas e como aplicá-las em seus protocolos profissionais.</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Button size="lg" className="bg-white text-indigo-700 hover:bg-white/90" asChild><Link to="/contato">Entrar em Contato</Link></Button>
              <Button variant="outline" size="lg" className="border-white/30 text-white hover:bg-white/10" asChild><Link to="/consultoria">Agendar Consultoria</Link></Button>
            </div>
          </div>
        </AnimatedSection>
      </main>
